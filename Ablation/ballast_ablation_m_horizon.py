...
df = pd.read_csv('m_ablation_utility_final_t5.csv', header=None)
...
t_grid_sample = np.arange(time_now, time_now + horizon+dt, dt)
...
# Save as one row
with open(f'./m_ablation_utility_gap_horizon{int(round(horizon * 10)):02d}.csv', 'a') as myfile:
...
# Save as one row
with open(f'./m_ablation_utility_id_horizon{int(round(horizon * 10)):02d}.csv', 'a') as myfile:
    np.savetxt(myfile, np.array(metric).reshape(1, -1), delimiter=',')
