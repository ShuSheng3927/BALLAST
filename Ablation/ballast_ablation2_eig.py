...
pos_sample_list = np.hstack((space_grid,np.repeat(t_choice,space_grid.shape[0]).reshape(-1,1)))
utility_comp_batched = jax.vmap(lambda row: utility_comp_jit(row[None, :]),
                                in_axes=0, out_axes=0)

values_total = utility_comp_batched(pos_sample_list)


df = pd.read_csv(f'm_ablation2_utility_final_t{t_choice}.csv', header=None)
overall_mean = np.array(df.loc[df[0] == key_choice,1::])[0]
best_value = np.max(overall_mean)

# Step 2: Compute utility gap at each sample size
utility_gaps = []
chosen_ids = []

for m in range(200):
    values_mean = values_total
    chosen_id = np.argmax(values_mean)
    chosen_ids.append(chosen_id)
    chosen_value = overall_mean[chosen_id]
...
# Save as one row
with open(f'./m_ablation2_utility_gap_eig_t{t_choice}.csv', 'a') as myfile:
...
# Save as one row
with open(f'./m_ablation2_utility_id_eig_t{t_choice}.csv', 'a') as myfile:
    np.savetxt(myfile, np.array(metric).reshape(1, -1), delimiter=',')
