...
# Save as one row
with open(f'./m_ablation2_suntans_utility_gap_t{t_choice}.csv', 'a') as myfile:
...
# Save as one row
with open(f'./m_ablation2_suntans_utility_id_t{t_choice}.csv', 'a') as myfile:
    np.savetxt(myfile, np.array(metric).reshape(1, -1), delimiter=',')

metric = jnp.hstack([jnp.array([seednum]), jnp.array(overall_mean)]) 
# Save as one row
with open(f'./m_ablation2_suntans_utility_final_t{t_choice}.csv', 'a') as myfile:
    np.savetxt(myfile, np.array(metric).reshape(1, -1), delimiter=',')