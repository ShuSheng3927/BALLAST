...
df = pd.read_csv(f'./m_ablation_utility_final_t{t_choice}.csv', header=None)
...
# Save as one row
with open(f'./m_ablation_utility_gap_eig_t{t_choice}.csv', 'a') as myfile:
...
# Save as one row
with open(f'./m_ablation_utility_id_eig_t{t_choice}.csv', 'a') as myfile:
    np.savetxt(myfile, np.array(metric).reshape(1, -1), delimiter=',')
