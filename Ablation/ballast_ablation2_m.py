...
key_choice = int(sys.argv[1])
key = jr.PRNGKey(key_choice)
...
for existing_id in existing_ids:
    ...
    curr_loc = traj_pos_full[traj_pos_full[:, 3] == existing_id, 0:2][-1]
    curr_loc_idx = find_loc_idx(space_grid, curr_loc)
    pos_ext, _ = initialise_drifter_traj(seed, dataset_ground_truth, space_grid, t_grid, curr_loc_idx, start_time_idx_sample, term_time_idx_sample, obs_noise, drifter_id)
    if pos_ext is not None:
        existing_traj_pos.append(pos_ext)
if len(existing_traj_pos) > 0:
    existing_block = jnp.vstack(existing_traj_pos)              # (E_tot, 4)
...
def sample_drifter(start_loc_idx):
    pos_sample, vel_sample = initialise_drifter_traj(seed, dataset_ground_truth, space_grid, t_grid, start_loc_idx, start_time_idx_sample, term_time_idx_sample, obs_noise, drifter_id)
    pos_sample = pos_sample.reshape(-1,4)
    vel_sample = vel_sample.reshape(-1,4)
    count = pos_sample.shape[0]
...
for idx in range(space_grid.shape[0]):
    pos_padded, count = sample_drifter(idx)
    pos_padded_list.append(pos_padded)
    count_list.append(count)

# stack into JAX arrays
pos_padded_batch = jnp.stack(pos_padded_list, axis=0)
count_batch = jnp.array(count_list)

overall_mean = jax.vmap(candidate_utility)(pos_padded_batch, count_batch) 
...
metric = jnp.hstack([jnp.array([key_choice]), jnp.array(utility_gaps)])            # shape (626,)

# Save as one row
with open(f'./m_ablation2_utility_gap_t{t_choice}.csv', 'a') as myfile:
    np.savetxt(myfile, np.array(metric).reshape(1, -1), delimiter=',')

metric = jnp.hstack([jnp.array([key_choice]), jnp.array(chosen_ids)])            # shape (626,)

# Save as one row
with open(f'./m_ablation2_utility_id_t{t_choice}.csv', 'a') as myfile:
    np.savetxt(myfile, np.array(metric).reshape(1, -1), delimiter=',')

metric = jnp.hstack([jnp.array([key_choice]), jnp.array(overall_mean)]) 
# Save as one row
with open(f'./m_ablation2_utility_final_t{t_choice}.csv', 'a') as myfile:
    np.savetxt(myfile, np.array(metric).reshape(1, -1), delimiter=',')