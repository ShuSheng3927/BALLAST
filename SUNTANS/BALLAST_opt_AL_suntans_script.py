...
def initialise_gp(kernel, mean, dataset):
    ...
    likelihood = gpx.likelihoods.Gaussian(
        num_datapoints=dataset.n, obs_stddev=gpx.parameters.Static(obs_noise)
    )
...
def full_system_EIG_BALLAST(seed, traj_pos_full, traj_vel_full, space_grid, time_now, BALLAST_samples, opt=False):
    ...
    kernel = HelmholtzKernelSpatioTemporal_derivative(
        potential_kernel=gpx.kernels.RBF(variance=20.0, lengthscale=5.0),
        stream_kernel=gpx.kernels.RBF(variance=0.01, lengthscale=4.0),
        time_kernel=Matern32new(active_dims=[3], lengthscale=1.0, variance=15.0),
    )
    ...
    if opt:
        ...
        def make_bounds(path, leaf):
            ...
            if re.search(r"prior/kernel/time_kernel/variance$", p):
                return (_like(10.0, like), _like(20.0, like))
            ...
            if re.search(r"prior/kernel/stream_kernel/variance$", p):
                return (_like(1e-1, like), _like(5.0, like))
...
for deploy in range(deploy_num):
    ...
    # acquisition
    acq_list = full_system_EIG_BALLAST(seed, traj_pos_full, traj_vel_full, space_grid, time_now, BALLAST_samples=20, opt=True)
...
with open('./traj_data_BALLAST20_opt_suntans.csv', 'a') as myfile:
...
metric = jnp.array([jnp.array(bias_list_full)])
metric = jnp.hstack([metric, jnp.repeat(seednum,metric.shape[0]).reshape(-1,1)])
with open('./metric_BALLAST20_opt_suntans.csv', 'a') as myfile:
