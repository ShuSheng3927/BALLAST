...
def full_system_len_dist_BALLAST(seed, traj_pos_full, traj_vel_full, space_grid, time_now, BALLAST_samples):
    # ---- GP setup (no jitting here) ----
    traj_data_5d = dataset_5d(traj_pos_full.T[0:3, :], traj_vel_full.T[0:2, :])
    mean_fn = gpx.mean_functions.Zero()
    kernel = HelmholtzKernelSpatioTemporal_derivative(
...
for deploy in range(deploy_num):
    ...
    # acquisition
    acq_list = full_system_len_dist_BALLAST(seed, traj_pos_full, traj_vel_full, space_grid, time_now, BALLAST_samples=20)
...
with open('./traj_data_BALLAST20_heuristic_suntans.csv', 'a') as myfile:
...
with open('./metric_BALLAST20_heuristic_suntans.csv', 'a') as myfile:
