...
def sobol_grid_indices(N, M, seed=None):
    """
    Return M unique flat indices into an N x N grid produced like your space_grid.
    The mapping matches your meshgrid/reshape order (row-major flattening).
    """
    if M > N*N:
        raise ValueError(f"Requested M={M} exceeds total grid cells N^2={N*N}.")

    sampler = qmc.Sobol(d=2, scramble=True, seed=seed)

    # We'll draw a sufficiently large Sobol block, map to cells, and dedupe in-order.
    # If collisions reduce the unique count, we'll increase the block size.
    ...
    # Start with a modest oversample; grow if needed.
    m = int(np.ceil(np.log2(max(1, int(M * 1.5)))))  # 2^m samples
    while True:
        # Draw 2^m Sobol points in [0,1)^2
        sob = sampler.random_base2(m=m)   # shape: (2**m, 2)

        # Map to integer grid indices (j for x, i for y), 0..N-1
        # sob[:,0] -> x (columns), sob[:,1] -> y (rows)
        ij = np.floor(sob * N).astype(int)
        j = ij[:, 0]
        i = ij[:, 1]

        # Flat index consistent with your space_grid construction:
        # meshgrid with shape (N, N) flattened row-major -> flat = i*N + j
        flat_idx = i * N + j

        uniq = ordered_unique(flat_idx)
        if uniq.size >= M:
            return jnp.array(uniq[:M])

        # Not enough unique cells; increase block size and try again
        m += 1
#%%
...
sobol_indices = sobol_grid_indices(N, deploy_num, seed=seednum)
...
for deploy in range(deploy_num):
    ...
    next_obs_loc_idx = sobol_indices[deploy] #jr.choice(seed, N_space, (1,))[0]#jnp.argmax(acq_list)
...
with open('./traj_data_SOBOL.csv', 'a') as myfile:
...
with open('./metric_SOBOL.csv', 'a') as myfile:
