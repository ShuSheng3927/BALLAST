...
def se_kernel(x,y,l):
    ...
    ## k_SE(x, y) = exp[- || x-y ||^2 / (2l^2)]

    ## arguments
    # l         length-scale
    # x, y      inputs
    return 0.5*jnp.exp(-jnp.linalg.norm(x-y)**2 / (2 * l**2))
def vectorize_kernel(kernel, l=None):
    if l is not None:
        kernel = partial(kernel, l=l)
    return jax.vmap(jax.vmap(kernel, in_axes=(None, 0)), in_axes=(0, None))

def par_11_se_kernel(x,y,l):
    ## \partial^2_{x1, x1'} k(x, x')
    return se_kernel(x,y,l) / l**2 - ((x[0] - y[0])**2 / l**4 )* se_kernel(x,y,l)

def par_22_se_kernel(x,y,l):
    ## \partial^2_{x2, x2'} k(x, x')
    return se_kernel(x,y,l) / l**2 - ((x[1] - y[1])**2 / l**4 )* se_kernel(x,y,l)

def par_12_se_kernel(x,y,l):
    ## \partial^2_{x1, x2'} k(x, x')
    return -((x[0] - y[0])*(x[1]-y[1]) / l**4 )* se_kernel(x,y,l)

def par_21_se_kernel(x,y,l):
    ## \partial^2_{x2, x1'} k(x, x')
    return -((x[0] - y[0])*(x[1]-y[1]) / l**4 )* se_kernel(x,y,l)

def quiver_format(uv_val):
    grid_points = uv_val.shape[0] / 2
    
    # assume square lattice
    u_points = jnp.sqrt(grid_points).astype(int)
    v_points = u_points

    u_val = uv_val[0:(u_points*v_points)].reshape((u_points*v_points),1)
    v_val = uv_val[(u_points*v_points)::].reshape((u_points*v_points),1)

    return u_val, v_val

l_phi = 0.8
l_psi = 0.5
phi_switch = 1 #divergence
psi_switch = 1 #curl

true_param = jnp.array([l_phi,l_psi])

k_uu_base = lambda x, y: par_11_se_kernel(x,y,l_phi)*phi_switch + par_22_se_kernel(x,y,l_psi)*psi_switch
k_vv_base = lambda x, y: par_22_se_kernel(x,y,l_phi)*phi_switch + par_11_se_kernel(x,y,l_psi)*psi_switch
k_uv_base = lambda x, y: par_12_se_kernel(x,y,l_phi)*phi_switch - par_21_se_kernel(x,y,l_psi)*psi_switch
k_vu_base = lambda x, y: par_21_se_kernel(x,y,l_phi)*phi_switch - par_12_se_kernel(x,y,l_psi)*psi_switch
k_uu = vectorize_kernel(k_uu_base)
k_vv = vectorize_kernel(k_vv_base)
k_uv = vectorize_kernel(k_uv_base)
k_vu = vectorize_kernel(k_vu_base)
...
range_x = [-2,2]
range_y = [-2,2]
x_grid = jnp.linspace(range_x[0], range_x[1], N)
y_grid = jnp.linspace(range_y[0], range_y[1], N)
x1test, x2test = jnp.meshgrid(x_grid, y_grid)
x1test = x1test.reshape(N**2,1)
x2test = x2test.reshape(N**2,1)
xtest = jnp.hstack((x1test, x2test))
space_grid = xtest
xtest_full = jnp.vstack([xtest, xtest])

mean = jnp.zeros(N*N*2)
cov_uu = k_uu(xtest, xtest)
cov_vv = k_vv(xtest, xtest)
cov_uv = k_uv(xtest, xtest)
cov_vu = jnp.transpose(cov_uv)
cov = jnp.block([
    [cov_uu, cov_uv],
    [cov_vu, cov_vv]
]) 
cov_matrix = cov + jnp.eye(cov.shape[0],cov.shape[1])*1e-10
...
t_grid = jnp.arange(0, time_final + dt, dt)
...
# ----- initial state f0: (cov ⊗ P_inf)^{1/2} * z -----
key = jr.PRNGKey(int(sys.argv[1]) // 10)
key, subkey = jr.split(key)
f_mat = sample_kron_cov(L_cov, L_Pinf, subkey)                     # 2 x n_blocks
y = f_mat[0, :].copy()                                     # H * f per block == first row
f_list = [f_mat.copy()]
y_list = [y.copy()]

# ----- time stepping -----
# (I ⊗ Phi) vec(f) == vec(Phi @ f_mat); so update is left-multiply by Phi
for _ in range(n_steps - 1):
    # process noise with covariance (cov ⊗ Q)
    key, subkey = jr.split(key)
    e_mat = sample_kron_cov(L_cov, L_Q, subkey)                    # 2 x n_blocks
    f_mat = Phi @ f_mat + e_mat                            # 2 x n_blocks
    f_list.append(f_mat.copy())
    y_list.append(f_mat[0, :].copy())                      # H selects first component

# ----- assemble datasets (match your original shapes) -----
# pos_stacked: stack [x, y, t] for all time steps, then transpose
pos_blocks = []
for t in t_grid:
    pos_blocks.append(
        np.hstack([space_grid, np.full((space_grid.shape[0], 1), t)])
    )                                                      # (N^2, 3)
pos_stacked = np.vstack(pos_blocks).T                      # (3, n_steps*N^2)

vel_blocks = []
for y in y_list:
    vel_blocks.append(y.reshape(-1, N**2).T)               # (2, N^2) -> (N^2, 2)
vel_stacked = np.vstack(vel_blocks).T          
dataset_ground_truth = dataset_4d(pos_stacked, vel_stacked)
...
def _tree_clip(params, bounds):
    # bounds is a pytree of tuples (lo, hi) broadcasting to each leaf
    def _clip_leaf(p, b):
        lo, hi = b
        return jnp.clip(p, lo, hi)
    return jtu.tree_map(_clip_leaf, params, bounds)
...
def fit_lbfgs(
    *,
    ...
    gtol: float = 1e-5,
    bounds: tp.Optional[tp.Any] = None,   # NEW: pytree of (lo, hi) per leaf
    project_in_step: bool = True,         # NEW: project after each update
    ...
    if use_bounds and params_bijection is not None:
        # You can also choose to allow both by composing, but the clean/robust
        # route for *closed intervals* is to skip the bijection.
        params_bijection = None

    # Start from a feasible point when bounds are provided
    if use_bounds:
        params = _tree_clip(params, bounds)
    else:
        if params_bijection is not None:
            params = transform(params, params_bijection, inverse=True)

    # Loss definition
    def loss(unconstrained_or_constrained_params: nnx.State) -> ScalarFloat:
        if use_bounds:
            constrained = _tree_clip(unconstrained_or_constrained_params, bounds)
        else:
            constrained = transform(unconstrained_or_constrained_params, params_bijection)
        model_eval = nnx.merge(graphdef, constrained, *static_state)
        return objective(model_eval, train_data)
    ...
    def step(carry):
        ...
        if use_bounds and project_in_step:
            params = _tree_clip(params, bounds)
...
def full_system_EIG(traj_data, space_grid, time_now):
    X_n = traj_data.X
    EIG_list = []

    for space in space_grid:
        new_obs_point = jnp.hstack([jnp.tile(jnp.hstack([space,jnp.array([time_now])]),2).reshape(2,-1),jnp.array([[0.],[1.]])])
        extended_X_n = jnp.vstack([X_n, new_obs_point])
        kernel = HelmholtzKernelSpatioTemporal(potential_kernel=gpx.kernels.RBF(variance=0.5,lengthscale=l_phi), stream_kernel=gpx.kernels.RBF(variance=0.5,lengthscale=l_psi),time_kernel=gpx.kernels.Matern32(lengthscale=l,variance=sigma2))
        gram = kernel.gram(extended_X_n).to_dense() 
        s, log = jnp.linalg.slogdet(jnp.eye(gram.shape[0]) + obs_noise**2 * gram)
        EIG_list.append(s*log)
    
    return jnp.array(EIG_list)
...
def full_system_EIG_BALLAST(seed, traj_pos_full, traj_vel_full, space_grid, time_now, BALLAST_samples, opt=False):
    traj_data_5d = dataset_5d(traj_pos_full.T[0:3,:], traj_vel_full.T[0:2,:])
    mean_fn = gpx.mean_functions.Zero()
    kernel = HelmholtzKernelSpatioTemporal_derivative(potential_kernel=gpx.kernels.RBF(variance=0.5,lengthscale=l_phi), stream_kernel=gpx.kernels.RBF(variance=0.5,lengthscale=l_psi),time_kernel=Matern32new(active_dims=[3],lengthscale=l,variance=sigma2))
    posterior = initialise_gp(kernel, mean_fn, traj_data_5d)

    if opt:
        # Peek at trainables to shape-match bounds
        graphdef, trainables, *_ = nnx.split(posterior, Parameter, ...)

        def _value_like(leaf):
            # If it's a VariableState, grab its underlying array for shape/dtype
            return getattr(leaf, "value", leaf)

        def _like(val, like_arr):
            val = jnp.asarray(val, dtype=jnp.asarray(like_arr).dtype)
            return jnp.broadcast_to(val, jnp.asarray(like_arr).shape)

        def make_bounds(path, leaf):
            like = _value_like(leaf)
            p = "/".join(map(str, path))   # e.g. 'prior/kernel/time_kernel/lengthscale'

            # ---- customize per-parameter boxes here ----
            if re.search(r"prior/kernel/time_kernel/lengthscale$", p):
                return (_like(1e-1, like), _like(3.0, like))

            if re.search(r"prior/kernel/time_kernel/variance$", p):
                return (_like(1e-1, like), _like(1.0, like))

            if re.search(r"prior/kernel/stream_kernel/lengthscale$", p):
                return (_like(1e-1, like), _like(1.0, like))

            if re.search(r"prior/kernel/stream_kernel/variance$", p):
                return (_like(1e-1, like), _like(1.0, like))

            if re.search(r"prior/kernel/potential_kernel/lengthscale$", p):
                return (_like(1e-1, like), _like(1.0, like))

            if re.search(r"prior/kernel/potential_kernel/variance$", p):
                return (_like(1e-1, like), _like(1.0, like))

            # default (or make unconstrained with ±inf)
            return (_like(1e-3, like), _like(5.0, like))

        bounds = jax.tree_util.tree_map_with_path(make_bounds, trainables)

        objective = lambda p, d: -gpx.objectives.conjugate_mll(p, d)
        opt_posterior, history = fit_lbfgs(
            model=posterior,
            objective=objective,
            train_data=traj_data_5d,
            bounds=bounds,          # ← closed intervals enforced
        )
        graphdef, trainables, *_ = nnx.split(opt_posterior, Parameter, ...)
        param_val = np.concatenate([x.ravel() for x in jax.tree_util.tree_leaves(trainables)])
    else:
    ...
    return eig_ballast_list, param_val
#%%
...
for deploy in range(deploy_num):
    seed, seed1 = jr.split(seed)
    # acquisition
    acq_list, param_val = full_system_EIG_BALLAST(seed, traj_pos_full, traj_vel_full, space_grid, time_now, BALLAST_samples=20,opt=True)
...
with open('./traj_data_BALLAST20_opt.csv', 'a') as myfile:
...
@jax.jit
def bias_compute(mean, true_field):
    return jnp.linalg.norm(mean - true_field, axis=1)
...
for d in deployment_time:
    ...
    for mt in metric_eval_time:
        mask = np.where(dataset_ground_truth.X[:,2] == t_grid[mt])[0]  # NumPy indices
        X_query = dataset_ground_truth.X[mask, :]
        y_true  = dataset_ground_truth.y[mask].flatten().reshape(-1, 2)

        gp = initialise_gp(kernel, mean_fn, curr_data)
        mean = gp.predict(X_query, train_data=curr_data).mean.reshape(-1, 2)

        bias = bias_compute(mean, y_true)
...
with open('./metric_BALLAST20_opt.csv', 'a') as myfile:
