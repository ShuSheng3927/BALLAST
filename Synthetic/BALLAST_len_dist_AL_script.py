...
def rank_array(x: jnp.ndarray, ascending: bool = True) -> jnp.ndarray:
    """
    Compute average ranks for a 1D array (like scipy.stats.rankdata(method="average")).
    
    Args:
        x: (N,) array
        ascending: if True, smallest value gets rank 1 (default).
                   if False, largest value gets rank 1.
    
    Returns:
        (N,) array of ranks (float), with ties averaged.
    """
    N = x.shape[0]

    # Flip sign if descending
    vals = x if ascending else -x

    # Sort values and get inverse permutation
    ...
    diffs = jnp.concatenate([jnp.array([True]), sorted_vals[1:] != sorted_vals[:-1]])
    group_id = jnp.cumsum(diffs) - 1   # assign group index

    # Compute mean rank per group
    group_means = jax.ops.segment_sum(positions, group_id, num_segments=group_id[-1] + 1) \
                  / jax.ops.segment_sum(jnp.ones_like(positions), group_id, num_segments=group_id[-1] + 1)

    # Assign back to original order
    sorted_ranks = group_means[group_id]
    return sorted_ranks[inv_order]

@jax.jit
def distance_compute(pos_padded_batch: jnp.ndarray, count_batch: jnp.ndarray) -> jnp.ndarray:
    """
    pos_padded_batch: shape (B, T, D>=2)
    count_batch:      shape (B,)  — number of valid points per batch item (<= T)
    returns:          shape (B,)  — total path length per batch item
    """
    B, T, _ = pos_padded_batch.shape

    # Clip counts to [0, T] to be safe
    counts = jnp.clip(count_batch, 0, T)

    # Take only the first two coords (x,y); shape (B, T, 2)
    coords = pos_padded_batch[..., :2]

    # Consecutive diffs along time axis; shape (B, T-1, 2)
    diffs = coords[:, 1:, :] - coords[:, :-1, :]

    # L2 distance per step; shape (B, T-1)
    step_dists = jnp.linalg.norm(diffs, axis=-1)

    # Mask out padded steps: for each item, only first (count-1) steps are valid
    valid_steps = jnp.maximum(counts - 1, 0)  # shape (B,)
    idx = jnp.arange(T - 1)                   # shape (T-1,)
    mask = (idx[None, :] < valid_steps[:, None])  # shape (B, T-1), bool

    # Sum only valid steps
    return (step_dists * mask.astype(step_dists.dtype)).sum(axis=1)

def full_system_len_dist_heuristic_BALLAST(seed, traj_pos_full, traj_vel_full, space_grid, time_now, BALLAST_samples):
    traj_data_5d = dataset_5d(traj_pos_full.T[0:3,:], traj_vel_full.T[0:2,:])
    mean_fn = gpx.mean_functions.Zero()
    kernel = HelmholtzKernelSpatioTemporal_derivative(potential_kernel=gpx.kernels.RBF(variance=0.5,lengthscale=l_phi), stream_kernel=gpx.kernels.RBF(variance=0.5,lengthscale=l_psi),time_kernel=Matern32new(active_dims=[3],lengthscale=l,variance=sigma2))
    posterior = initialise_gp(kernel, mean_fn, traj_data_5d)
    ...
    def compute_min_distances(grid_points: jnp.ndarray, obs_points: jnp.ndarray) -> jnp.ndarray:
        """
        Compute min Euclidean distance from each grid point to observation set.
        
        grid_points: (N, 2) array
        obs_points:  (M, 4) array, only first 2 columns are used, with duplicates
        """
        # Take only unique first two cols of obs_points
        obs_xy = jnp.unique(obs_points[:, :2], axis=0)  # shape (M_unique, 2)

        # Expand dimensions for broadcasting
        grid_exp = grid_points[:, None, :]   # (N, 1, 2)
        obs_exp  = obs_xy[None, :, :]        # (1, M, 2)

        # Compute squared Euclidean distances
        dists_sq = jnp.sum((grid_exp - obs_exp) ** 2, axis=-1)  # (N, M)

        # Take minimum distance per grid point
        min_dists = jnp.sqrt(jnp.min(dists_sq, axis=1))  # (N,)

        return min_dists

    min_seperation = compute_min_distances(space_grid, X_n)
    sep_rank = rank_array(min_seperation, ascending=True)
    ...
    distance_rank_list = jnp.zeros(space_grid.shape[0])
    for m in range(BALLAST_samples):
        max_m = int((term_time_idx_sample - start_time_idx_sample) / 5) + 1
        velocity_lookup = one_posterior_trajectory(jr.PRNGKey(m), helmholtz_mean, helmholtz_cov_chol)
        ...
        start_loc_idxs = jnp.arange(space_grid.shape[0])
        pos_padded_batch, count_batch = jax.vmap(sample_drifter)(start_loc_idxs)  # shapes: (N^2, max_m, 4), (N^2,)
        distance_rank = rank_array(distance_compute(pos_padded_batch, count_batch), ascending=False)
        distance_rank_list = distance_rank_list + distance_rank

    distance_rank_list = distance_rank_list / BALLAST_samples
    total_rank = distance_rank_list + sep_rank
...
for deploy in range(deploy_num):
    ...
    # acquisition
    acq_list = full_system_len_dist_heuristic_BALLAST(seed, traj_pos_full, traj_vel_full, space_grid, time_now, BALLAST_samples=20)
...
with open('./traj_data_BALLAST20_heuristic.csv', 'a') as myfile:
...
with open('./metric_BALLAST20_heuristic.csv', 'a') as myfile:
